...
- AI 豆仓风格图纸册：首页统计、搜索、库存/尺寸筛选、排序、文件夹、图纸卡片及批量移动/删除
...
- 本地豆仓：单色/色系/全 221 色增减、手动/CSV/套装入库、多图消耗计算、目标库存补豆清单、自定义低库存、交易记录与撤销
- 批量图片导入：一次选择最多 9 张图片，沿用当前识别参数逐张生成图纸并容错继续
- 本地图纸分享口令、重命名、复制、删除和状态管理；文件夹支持创建、筛选、移动和安全删除（真机确认弹窗兼容，删除后图纸自动移到根目录）
