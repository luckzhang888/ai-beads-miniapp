...
必须先识别完整网格面积 grid/perspective，再确定 rows、columns。标题明确写出 99×106 时必须原样返回并把 dimensionSource 设为 title，不得因漏检一条浅色边线改成 98×106。细小网格不能准确计数且标题也不可读时返回 null 并写 warnings，绝不编造行列。标题中的尺寸、色数、总颗数只在文字明确可读时填写。若标题声明了色数和总颗数，请逐项放大检查底部或侧边图例；只有 legendEntries 条数等于 declaredColorCount 且所有 count 之和等于 declaredBeadCount 时才返回完整 legendEntries，否则保留能确认的条目并在 warnings 说明缺失，禁止凑数。不要把图例色号混入 detectedCodes，也不要返回完整色号矩阵。无网格的照片或插画必须返回 hasGrid=false、grid=null、perspective=null，不能虚构网格。`
